...
namespace http
{
    ...
    inline namespace detail
    {
        ...
        constexpr int getAddressFamily(InternetProtocol internetProtocol)
        {
            return (internetProtocol == InternetProtocol::V4) ? AF_INET :
                ...
                throw RequestError(xorstr_("Unsupported protocol"));
        }
    }
    ...
    public:
            ...
            const std::vector<std::string>& headers = {})
        {
            ...
            std::string requestData = method + " " + path + xorstr_(" HTTP/1.1\r\n");

            for (const std::string& header : headers)
                requestData += header + "\r\n";

            requestData += xorstr_("Host: ") + domain + xorstr_("\r\n");
            requestData += xorstr_("Content-Length: ") + std::to_string(body.size()) + xorstr_("\r\n");
            ...
            while (remaining > 0)
            {
                ...
                if (size < 0)
                    throw std::system_error(getLastError(), std::system_category(), xorstr_("Failed to send data to ") + domain + xorstr_(":") + port);
            ...
            for (;;)
            {
                ...
                if (size < 0)
                    throw std::system_error(getLastError(), std::system_category(), xorstr_("Failed to read data from ") + domain + xorstr_(":") + port);
                else if (size == 0)
                    break; // disconnected
                    ...
                    for (;;)
                    {
                        ...
                        else // headers
                        {
                            ...
                            if (pos != std::string::npos)
                            {
                                ...
                                else if (headerName == xorstr_("Transfer-Encoding"))
                                {
                                    if (headerValue == xorstr_("chunked"))
                                        chunkedResponse = true;
                                    else
                                        throw ResponseError(xorstr_("Unsupported transfer encoding: ") + headerValue);
                                }
                            }
                        }
                    }
