...
namespace aidsware::ui
{
	...
	namespace wrapper
	{
		...
		{
			ptrnBuilder.Widget(vars::Container).Title(name).Position(position.x + currentX, position.y).Size(size.x, size.y).Key(menu_key).Font(xorstr_("MinecraftCHMC"), 20, true, 1);
		}
		...
		void button(std::string name, std::function<void()> callback, Vector2 position, Tabs tab = Tabs::Visual)
		{
			...
			ptrnBuilder.Widget(vars::buttons[name]).Title(name).Position(position.x + currentX, currentY + position.y).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).Callback(callback).SpawnIn(vars::Container, false);
			currentY += 25.0f;
		}
		...
		void checkbox(std::string name, Vector2 position, Tabs tab = Tabs::Visual, bool keybind = false, bool colorpick = false, Vector2 cppos = Vector2(0,0))
		{
			...
			auto cb = ptrnBuilder.Widget(vars::checkBoxes[name]).Title(name).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			if (colorpick)
			{
				vars::colorPickers[name] = std::make_shared<FGUI::CColorPicker>();
				ptrnBuilder.Widget(vars::colorPickers[name]).Title(name + xorstr_(" color")).Position((text_size.x * 1.5) + 10.0f + position.x + cppos.x, currentY + position.y).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			}
			currentY += 20.0f;
			if (keybind)
			{
				vars::keybinders[name + xorstr_(" key")] = std::make_shared<FGUI::CKeyBinder>();
				ptrnBuilder.Widget(vars::keybinders[name + xorstr_(" key")]).Title(name + xorstr_(" key")).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
				currentY += 25.0f;
			}
			currentY += 5.0f;
		}
		...
		void label(std::string name, Vector2 position, Tabs tab = Tabs::Visual, bool newline = true, bool fonticon = false, std::string e = "")
		{
			...
			ptrnBuilder.Widget(vars::labels[name]).Title(name).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			if(newline)
				currentY += 20.0f;
		}
		...
		{
			vars::colorPickers[name] = std::make_shared<FGUI::CColorPicker>();
			ptrnBuilder.Widget(vars::colorPickers[name]).Title(name).Position(position.x + currentX, currentY + position.y).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			currentY += 20.0f;
		}
		...
		void textbox(std::string name, Vector2 position, Tabs tab = Tabs::Visual, std::string value = "")
		{
			...
			ptrnBuilder.Widget(vars::textBoxes[name]).Title(name).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Text(value).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			currentY += 25.0f;
		}
		...
		void combobox(std::string name, std::vector<std::string> options, Vector2 position, Tabs tab = Tabs::Visual)
		{
			...
			auto widget = ptrnBuilder.Widget(vars::comboBoxes[name]).Title(name).Position(position.x + currentX, currentY + position.y).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab);
		...
		void keybind(std::string name, Vector2 position, Tabs tab = Tabs::Visual)
		{
			...
			ptrnBuilder.Widget(vars::keybinders[name]).Title(name).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			currentY += 25.0f;
		}
		...
		void slider(std::string name, Vector2 position, float value, float min, float max, Tabs tab = Tabs::Combat)
		{
			...
			ptrnBuilder.Widget(vars::sliders[name]).Title(name).Range(min, max).Value(value).Position(position.x + currentX, position.y + currentY).Font(xorstr_("MinecraftCHMC"), 20).Medium(vars::Tabs, (int)tab).SpawnIn(vars::Container, false);
			currentY += 30.0f;
		}

		void tabs(std::vector<std::string> tabs)
		{
			auto builder = ptrnBuilder.Widget(vars::Tabs).Position(0, 0).Font(xorstr_("MinecraftCHMC"), 16, true, 1);
			for (auto tab : tabs)
				builder.Tab(tab);
